...
class App extends React.Component{
  ...
  handleTermChange(term){
    if(term==''){
      const url = 'http://api.giphy.com/v1/gifs/trending?api_key=7X5CFD0oV8ylPWfiLtsUkcFNuBy50raw&limit=5';
      request.get(url, (err, res) => {
        this.setState({ gifs: res.body.data })
      });
    }
    else{
      const url = `http://api.giphy.com/v1/gifs/search?q=${term}&api_key=7X5CFD0oV8ylPWfiLtsUkcFNuBy50raw&limit=5`;
      request.get(url, (err, res) => {
        this.setState({ gifs: res.body.data })
      });
    }
